		  ...
		  m_wavetableIndex(0)
{
	...
	m_projectionMatrix = glm::ortho(0.f, static_cast<float>(m_viewport.x), 0.f, static_cast<float>(m_viewport.y), -0.1f,
			10.f);
	...
	if (m_shader.loadFromString(vertex, fragment))
	{
		...
		glCheck(glUniformMatrix4fv(uniforms.find("u_projectionMatrix")->second, 1, GL_FALSE,
