...
std::int32_t MapSystem::parseTileLayer(const tmx::Layer* layer, const tmx::Map& map, cro::Texture& /*out*/ mapTexture)
{
				...
				for (; i < tilesets.size(); ++i)
				{
					if (tiles[idx].ID >= tilesets[i].getFirstGID() && tiles[idx].ID <= tilesets[i].getLastGID())
					{
						...
						auto newPixels = utils::getTextureSubPixels(
								*textures[i], { static_cast<float>(tileX) * tileSize.x,
	...
	for (const auto& v: vertexArrays)
	{
		...
		auto oldPixels = utils::getTextureSubPixels(mapTexture,
				{ static_cast<float>(v.first.x), static_cast<float>(v.first.y) }, tileSize);
		...
		std::vector<ShapeInfo>& shapes)
{
	...
	if (name == "spawn")
	{
		...
		for (const auto& obj: objs)
		{
			...
			if (type == "spawnpoint")
			{
				spawnPoints[obj.getName()] = glm::vec2{ obj.getPosition().x + obj.getAABB().width / 2.f,
		...
		std::map<std::string, BackgroundElement>& elements)
{
	if (layer->getVisible())
	{
		...
		auto position = glm::vec2{ imgLayer->getOffset().x, map.getBounds().height - (float)imgLayer->getOffset().y -
